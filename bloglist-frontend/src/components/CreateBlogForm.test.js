...
describe('<CreateBlogForm />', () => {
    test('form calls event handler with the right details', async () => {
        ...
        expect(handleBlogCreating.mock.calls[0][0].title).toBe(
            'title for testing the form'
        )
        expect(handleBlogCreating.mock.calls[0][0].author).toBe(
            'author for testing the form'
        )
        expect(handleBlogCreating.mock.calls[0][0].url).toBe(
            'Url for testing the form'
        )
    })
})
