...
describe('<Blog />', () => {
    test('blog renders only title and author', async () => {
        ...
        const { container } = render(
            <Blog blog={blog} updateAppComponent={updateAppComponent} />
        )

        const div = container.querySelector('.briefInfo')
        expect(div).toHaveTextContent('blog for testing')
        expect(div).toHaveTextContent('fs developer')

        expect(div).not.toHaveTextContent('fullstack.com')

        expect(div).not.toHaveTextContent('5432')
    })
    ...
    test('url and number of likes are shown when button has been clicked', async () => {
        ...
        expect(div).toHaveTextContent('fullstack.com')

        expect(div).toHaveTextContent('5432')
    })
